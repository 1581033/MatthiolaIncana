...
const transform: AxiosTransform = {
  ...
  responseInterceptors: (res) => {
    if (res.headers?.[TOKEN_NAME]) {
      res.data.data = res.headers?.[TOKEN_NAME];
    }
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    switch (request.status) {
      case 0:
        //router.push({ path: '/result/network-error' });
        break;
      case 500:
        //router.push({ path: '/result/500' });
        break;
      default:
        break;
    }
  ...
  return new VAxios(
    merge(
      <CreateAxiosOptions>{
        ...
        authenticationScheme: 'CACHESESSIONID=',
        // 超时
        timeout: 10 * 1000,
        // 携带Cookie
