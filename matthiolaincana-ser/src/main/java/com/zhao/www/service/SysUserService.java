...
import com.zhao.www.mapper.SysUserMapper;
...
public class SysUserService extends ServiceImpl<SysUserMapper,SysUser> implements UserDetailsService {
    ...
    public UserInfoDto userInfo(HttpServletRequest request) {
        ...
        return baseMapper.selectByUserNameForRole(JwtTokenUtil.getTokenUserName(token,o));
    }
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        SysUser sysUser = Optional.ofNullable(getOne(wrapper)).orElseThrow(() -> new UsernameNotFoundException("用户名错误"));
        sysUser.setAuthorities(AuthorityUtils.commaSeparatedStringToAuthorityList("ROLE_ADMINS"));
        return sysUser;
    }
    ...
    public Authentication loadUserByToken(String token,String key) throws UsernameNotFoundException {
        ...
        SysUser sysUser = Optional.ofNullable(getOne(wrapper)).orElseThrow(() -> new UsernameNotFoundException("用户名错误"));
        sysUser.setAuthorities(AuthorityUtils.commaSeparatedStringToAuthorityList("ROLE_ADMINS"));
        return sysUser.getAuthentication();
    }
